...
async function oraSelect(req, res) {
    ...
    try {
        ...
        console.log("## execute Query\n"+ `SELECT STRUC FROM TB_JSP WHERE 1=1 AND NO = (SELECT NO FROM TB_PHP WHERE 1=1 AND STRUC = '${req.body.input}')`);

        var result = await connection.execute(`SELECT STRUC FROM TB_JSP WHERE 1=1 AND NO = (SELECT NO FROM TB_PHP WHERE 1=1 AND STRUC = :asis)`, [`${req.body.input}`]);
